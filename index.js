...
fetch('diseases.json', { cache: 'no-store' })
  ...
  .then(async json => {
    ...
    // ✅ LEGAL await now
    await enrichWithMedlinePlus(PACK.conditions);
...
async function enrichWithMedlinePlus(conditions) {
  for (const c of conditions) {
    const term = c.name || c.text;
    if (!term) continue;

    try {
      async function fetchFromDatabase(term) {
        const res = await fetch(`/api/condition?q=${encodeURIComponent(term)}`);
      ...
      if (m.medline) c.references.medline = m.medline;

      // ICD-10
      c.references.icd10 = Array.from(new Set([
        ...(Array.isArray(c.references.icd10) ? c.references.icd10 : []),
        ...(m.icd10 || [])
      ]));

    } catch (e) {
      console.warn("MedlinePlus failed for:", term);
    }
  }
}
...
function initApp() {
  ...
  async function fetchFromMedline(term) {
    const url =
    ...
    const get = name =>
      xml.querySelector(`content[name="${name}"]`)?.textContent || "";

    return {
      name: get("title"),
      aliases: get("alsoCalled")
        .split(";")
        .map(s => s.trim())
        .filter(Boolean),
      icd10: get("icd10cm")
